...
한 줄 요청이었지만, Claude 가 실제로 따른 작업 지시를 풀어 쓰면 아래와 같습니다. 다른 보고서에 그대로 붙여 쓸 수 있는 형태입니다.

{{< prompt title="PPT 생성 프롬프트 (재사용용)" >}}
아래 데이터 표를 근거로 인프라 팀장 10분 보고용 PPT를 만들어줘. pptxgenjs 스크립트로 .pptx 파일을 생성하고, validate → LibreOffice 렌더 → 이미지로 검수 → 결함 수정까지 해서 PowerPoint에서 바로 열리는 파일로 줘.

[데이터 표: 지표 / 수치 / 대상 그룹 / 페이지 — NotebookLM 데이터 표 42행 붙여넣기]

구성 (16:9, 8장):
1. 표지 — 제목 "인프라의 AI 준비도 격차", 부제에 보고서명과 "플랫폼팀 10분 보고"
2. 한 장 요약 — 큰 숫자 3개 (개발 속도 89% / 인프라 부담 86% / AI 유발 사고 93%)
3. AIMI 4단계 — 세로 막대 차트 (Exposed 24 / Fragmented 32 / Outpacing 25 / Pioneer 19) + 단계별 한 줄 설명
4. 거버넌스 역설 — 자신감 86% vs 공식 정책 30% (인프라 리더), Exposed 4%
5. 바이브 코딩 — 가로 막대 차트 (개발자 코드 79 / IaC 78 / 정책 코드 78 / 최소 검토 프로덕션 76)
6. 사고 — 차트 2개: '사고 없음' Pioneer 17% vs Exposed 3%, 사고 유형 37/36/36
7. Pioneer vs Exposed 비교표 — IaC 커버리지, 플랫폼 엔지니어링 완료, 공식 정책, 에이전틱 거버넌스 계획
8. 다음 분기 우선순위 3가지 + 진단 질문 3개 (어두운 배경으로 마무리)

규칙:
- 숫자마다 대상 그룹(전체 응답자 / 인프라 리더 / Pioneer / Exposed)을 작은 글씨로 붙일 것
- 슬라이드마다 하단에 출처와 페이지 번호 (Spacelift, The Infrastructure Automation Report 2026, n=406 · p.N)
- 차트는 이미지가 아니라 네이티브 차트(addChart)로. 값 레이블 표시, 범례 숨김
- 색: 남색 1E2761 기본, 주황 F59E0B 강조 하나, 빨강은 Exposed 에만. 폰트 Arial
- 텍스트 상자는 전부 isTextBox: true, margin: 0
- 제목은 한 줄에 들어오게, 넘치면 부제로 내릴 것
- 표지와 마지막 장은 어두운 배경, 나머지는 흰 배경
- 스피커 노트는 표지에 한 줄만
{{< /prompt >}}

Claude 는 이 지시대로 8장 구성을 잡습니다: 표지 → 한 장 요약(3 수치) → AIMI 4단계(차트) → 거버넌스 역설 → 바이브 코딩(차트) → 사고(차트 2) → Pioneer vs Exposed 비교표 → 다음 분기 3가지.
